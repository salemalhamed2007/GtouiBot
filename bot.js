...
//by ,$ ReBeL ء , 🔕#4777 'CODES SERVER'
	if (command === `play`) {
		...
		if (!permissions.has('SPEAK')) {
			return msg.channel.send('ل:x: **You have to be in a voice channel to use this command.**');
		}//by ,$ ReBeL ء , 🔕#4777 'CODES SERVER'
		...
		if (url.match(/^https?:\/\/(www.youtube.com|youtube.com)\/playlist(.*)$/)) {
			...
			return msg.channel.send(`:white_check_mark:**``${playlist.title}``** **The song has been added to the menu!.**`);
		} else {
			try {//by ,$ ReBeL ء , 🔕#4777 'CODES SERVER'
				...
				try {//by ,$ ReBeL ء , 🔕#4777 'CODES SERVER'
			        ...
			        .setDescription(`**Please enter your section number** **``1,2,3,4,5,6``** :
...
//by ,$ ReBeL ء , 🔕#4777 'CODES SERVER'
					...
					try {
						...
						console.error(err);
						return msg.channel.send(':x:**Bay** ***(Try again)**');
					}
		...
		return msg.channel.send(`:speaker: **Ok** **${args[1]}**`);
	} else if (command === `np`) {
		if (!serverQueue) return msg.channel.send(':x: **You have to be in a voice channel to use this command.**');
...
//by ,$ ReBeL ء , 🔕#4777 'CODES SERVER'
		try {
			...
			return msg.channel.send(`:x:**Error**.. ${error}`);
		}
	} else {//by ,$ ReBeL ء , 🔕#4777 'CODES SERVER'
		serverQueue.songs.push(song);
		console.log(serverQueue.songs);
		if (playlist) return undefined;
		else return msg.channel.send(`:white_check_mark:**``${song.title}``** **The song has been added to the menu!.**`);
	}
...
//by ,$ ReBeL ء , 🔕#4777 'CODES SERVER'
	...
	serverQueue.textChannel.send(`**:white_check_mark:  Searching :mag:** **``${song.title}``**`);
}//by ,$ ReBeL ء , 🔕#4777 'CODES SERVER'



client.on('message', Duy0 => {
    if (Duy0.content.startsWith('@GtouiBot#4061')) {
        Duy0.channel.send('**My prefix here is** **`=`**')
    }
});
...
//bot 
client.on('message', Duy0 => {
    if (Duy0.content.startsWith('=bot')) {
        Duy0.channel.send(':envelope: ***Bot Designer..*** **|** **``SLooMz``**:white_check_mark:')
    }
});

client.on('ready', () => {
    client.channels.get("491205242298564608").join();
    });
